...
interface LifestyleTrendChartProps {
  dataPoints: DailyHealthTrendPoint[];
  isLoading?: boolean;
}

export const LifestyleTrendChart: React.FC<LifestyleTrendChartProps> = ({
  dataPoints,
  isLoading = false,
}) => {
  if (isLoading) {
    return (
      <div className="h-64 flex items-center justify-center bg-slate-900/40 rounded-xl border border-slate-800 animate-pulse">
        <p className="text-xs text-slate-500">Loading lifestyle metrics...</p>
      </div>
    );
  }
  ...
  if (!dataPoints || dataPoints.length === 0) {
    return (
      <div className="h-64 flex flex-col items-center justify-center bg-slate-900/30 rounded-xl border border-slate-800 p-6 text-center">
        ...
        <p className="text-sm font-medium text-slate-300">No lifestyle trend data available</p>
        <p className="text-xs text-slate-500 max-w-sm mt-1">
          Log your sleep hours and stress levels during daily check-ins to monitor wellness correlations.
        </p>
      </div>
    );
  }
